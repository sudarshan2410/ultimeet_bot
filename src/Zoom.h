...
class Zoom : public Singleton<Zoom> {
    ...
    MeetingServiceEvent* m_meetingServiceEvent;
    ...
    function<void()> onJoin = [&]() {
        ...
        if (m_config.useRawRecording()) {
            Log::info("Checking useRawRecording: " + std::to_string(m_config.useRawRecording()));
...
public:
    ...
    // Add the following method to allow setting the meeting event
    void setMeetingServiceEvent(MeetingServiceEvent* event);
};
